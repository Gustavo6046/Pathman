 ...
 * @author gustavo6046
 *
 *         The PathmanDemo class is a demonstration of how Pathman can be
 *         applied for action planning in video game AIs.
 * 
 *         Try reading its source code to learn how to use Pathman!
 */
public class PathmanDemo extends DemoClass
{
	public static void main(String[] args) throws ClassNotFoundException, KeyException
	{
		...
		// inheriting class.
		Circumstance beginning = man.makeBaseCircumstance(Optional.of("beginning"),
				cbuilder.addCondition("castleFlank", 0).addCondition("distance", 50).addCondition("dead", 0)
						.addCondition("invading", 0).build());
		...
		Circumstance f1 = man.makeBaseCircumstance( // You can also store these!
				...
				cbuilder.addCondition("castleFlank", 1).addCondition("distance", 5).addCondition("dead", 1)
						.addCondition("invading", 1).build());

		Circumstance f2 = man.makeBaseCircumstance(Optional.of("invade flank 2"), // Entering Flank 2.
				cbuilder.addCondition("castleFlank", 2).addCondition("distance", 1.75).addCondition("dead", 0)
						.addCondition("invading", 1).build());

		Circumstance f3 = man.makeBaseCircumstance(Optional.of("invade flank 3"), // Entering Flank 3.
				cbuilder.addCondition("castleFlank", 3).addCondition("distance", 7).addCondition("dead", 0)
						.addCondition("invading", 1).build());

		Circumstance f4 = man.makeBaseCircumstance(Optional.of("invade flank 4"), // Entering Flank 4.
				cbuilder.addCondition("castleFlank", 4).addCondition("distance", 12).addCondition("dead", 1)
						.addCondition("invading", 1).build());
		...
		Circumstance change = man.makeBaseCircumstance(
				...
				null, cbuilder.addCondition("distance", 0).addCondition("invading", 1).build());

		// This is where we put those stored circumstances at use.
		Action f2_post = Action.partialAction(man.planSpace, null, // use origin equality as matcher
				f2, change, 2.5);

		Action f3_post = Action.partialAction(man.planSpace, null, // use origin equality as matcher
				f3, change, 7);

		man.addAction(f2_post);
		man.addAction(f3_post);

		man.addCircumstance(f2_post.result, // Getting the modified Circumstance
		...
		// best one :)
		man.addCircumstance(f3_post.result, Optional.of("victory! (flank 3)") // Going through Flank 3
		);
		...
		Circumstance.Matcher goal = Circumstance.matcher().addEqual("distance", 0).build();
